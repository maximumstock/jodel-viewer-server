...
module.exports = {
    ...
    find: function* find(next) {
      ...
      let query = knex('jodels').select('data').limit(100);

      if(this.query.sort) {
        if(this.query.sort === 'popular') query.orderByRaw('data->>\'vote_count\' desc')
        else if(this.query.sort === 'discussed') query.orderByRaw('data->>\'child_count\' desc')
