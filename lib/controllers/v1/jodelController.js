...
module.exports = {
    ...
    find: function* find(next) {
      ...
      let query = knex('jodels').where('parent', null).limit(100).whereRaw('created_at > NOW() - interval \'2 hours\'');

      if(this.query.sort) {
        if(this.query.sort === 'popular') query.orderBy('vote_count', 'desc')
        else if(this.query.sort === 'discussed') query.orderBy('child_count', 'desc')
